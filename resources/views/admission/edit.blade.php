    ...
    <section>
        <div class="container py-10">
            <div class="flex justify-between items-center mb-10">
                <h1 class="text-3xl font-semibold">Update Admissions</h1>
                <a class="rounded-2xl bg-(--primary) py-2 px-2 text-white" href="/admission/create">Back</a>
            </div>
            <form action="/admission/update/{{ $admission->id }}" method="POST" enctype="multipart/form-data">
                @method('patch')
                @csrf
                <div class="grid grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="text"name="name" id="name"
                            placeholder="eg.Laravel"value="{{ $admission->name }}"
                            class="border border-gray-400 w-full px-2 py-1">
                    </div>
                    <div>
                        <label for="email">Email</label>
                        <input type="text"name="email" id="email"value="{{ $admission->email }}"
                            class="border border-gray-400 w-full px-2 py-1">
                    </div>
                ...
                <div class="col-span-2">
                    ...
                    <input type="tel"name="phone" id="phone" value="{{ $admission->phone }}"
                        class="border border-gray-400 w-full px-2 py-1">
